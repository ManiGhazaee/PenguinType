...
function moveCaretElemToEnd() {
    if (caretStyleState === "BLOCK" && smoothCaretState === "OFF")
        return;
...
function addPunctuation(arr) {
    ...
    let addDotExeptions = [".", ",", ";", ":", "-", "!", "?"];
    if (addDotExeptions.indexOf(lastCharOfLastWord) === -1)
        arr[arr.length - 1] += ".";
    return arr;
}
...
function ctrlbackSpaceHandler() {
    ...
    if (isFirstChild(pos) && isFirstChild(parent))
        return;
    else {
...
function resultColoring() {
    ...
    if (wpm > resultBestWpm)
        wpmperc = 100;
    else if (wpm < resultWorstWpm)
        wpmperc = 0;
    if (rawWpm > resultBestWpmRaw)
        wpmrawperc = 100;
    else if (rawWpm < resultWorstWpmRaw)
        wpmrawperc = 0;
    if (accuracy > resultBestAcc)
        accperc = 100;
    else if (accuracy < resultWorstAcc)
        accperc = 0;
    if (errors > errdiff)
        errprec = 100;
    else if (errors <= 0)
        errprec = 0;
    if (consistency > resultBestConsis)
        conperc = 100;
    else if (consistency < resultWorstConsis)
        conperc = 0;
    let color1 = rgbToArray(hexToRgb(resultWorstColorHEX));
    let color2 = rgbToArray(hexToRgb(resultBestColorHEX));
    let wpmC = arrayToRgb(betweenTwoColor(color1, color2, wpmperc));
    let rawwpmC = arrayToRgb(betweenTwoColor(color1, color2, wpmrawperc));
    let accC = arrayToRgb(betweenTwoColor(color1, color2, accperc));
    let errC = arrayToRgb(betweenTwoColor(color2, color1, errprec));
    let conC = arrayToRgb(betweenTwoColor(color1, color2, conperc));
    ...
    if (consistency == "0%")
        docConsis.style.color = resultWorstColorHEX;
}
...
function updateCaretOnScreen() {
    if (caretStyleState === "block" && smoothCaretState === "OFF")
        ...
        let caret = document.querySelector(".block-caret");
...
function positionUpdate() {
    ...
    if (caretStyleState === "BLOCK") {
        if (smoothCaretState === "ON") {
            head.insertAdjacentHTML("beforeend", `<style id="new-position" type="text/css">
...
function liveRawWpmStart() {
    liveRawWpmInter = setInterval(() => {
        if (resultField.style.display === "block")
...
function liveRawWpmColoring(wpm) {
    ...
    if (wpm > resultBestWpmRaw)
        wpmrawperc = 100;
    else if (wpm < resultWorstWpmRaw)
        wpmrawperc = 0;
    let color1 = rgbToArray(hexToRgb(resultWorstColorHEX));
    let color2 = rgbToArray(hexToRgb(resultBestColorHEX));
...
function liveAccuracyColoring(acc) {
    ...
    if (acc > resultBestAcc)
        accperc = 100;
    else if (acc < resultWorstAcc)
        accperc = 0;
    let color1 = rgbToArray(hexToRgb(resultWorstColorHEX));
    let color2 = rgbToArray(hexToRgb(resultBestColorHEX));
